...
#if (ISMRMRD_VERSION_MAJOR != 1) | (ISMRMRD_VERSION_MINOR != 13) | (ISMRMRD_VERSION_PATCH != 0)
...
inline void save(ISMRMRD::CompressiblePortableBinaryOutputArchive &ar, ISMRMRD::ISMRMRD_Image const &image, const unsigned int version) {
    ISMRMRD::CompressionParameters &parameters = cereal::get_user_data<ISMRMRD::CompressionParameters>(ar);
