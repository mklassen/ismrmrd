...
uint (*get_encoder(uint dims, int32 *))(zfp_stream *, const int32 *) {
...
uint (*get_encoder(uint dims, int64 *))(zfp_stream *, const int64 *) {
...
uint (*get_decoder(uint dims, int32 *))(zfp_stream *, int32 *) {
...
uint (*get_decoder(uint dims, int64 *))(zfp_stream *, int64 *) {
...
demote(D *oblock, const S *iblock, uint dims) {
    ...
    if (std::is_signed<D>::value) {
        ...
        S offset = 1u << static_cast<unsigned int>(std::numeric_limits<D>::digits);
        while (count--) {
        ...
        S offset = 1u << static_cast<unsigned int>(std::numeric_limits<D>::digits - 1);
        while (count--) {
...
compress(zfp_stream *zfp, const zfp_field *field) {
    ...
    uint (*encode)(zfp_stream *, const D *) = get_encoder(dims, oblock);
                    ...
                    for (l = 0; l < bw; l++) {
                        for (k = 0; k < bz; k++) {
                            for (j = 0; j < by; j++) {
                                ...
                                pad_block(iblock + ((l * zfp_block_size + k) * zfp_block_size + j) * zfp_block_size + bx, fx, 1);
                            }
                            pad_block(iblock + ((l * zfp_block_size + k) * zfp_block_size + by) * zfp_block_size + bx, fy, zfp_block_size);
                        }
                        pad_block(iblock + ((l * zfp_block_size + bz) * zfp_block_size + by) * zfp_block_size + bx, fz, zfp_block_size * zfp_block_size);
                    }
                    pad_block(iblock + ((bw * zfp_block_size + bz) * zfp_block_size + by) * zfp_block_size + bx, fw, zfp_block_size * zfp_block_size * zfp_block_size);
...
decompress(zfp_stream *zfp, zfp_field *field) {
    ...
    uint (*decode)(zfp_stream *, D *) = get_decoder(dims, iblock);
