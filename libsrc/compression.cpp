...
void decompress_acquisition(ISMRMRD::ISMRMRD_Acquisition &acq, std::vector<uint8_t> &buffer) {
    ...
    case 1:
        ...
        break;
    case 2:
        zfp_field_set_stride_2d(field.get(), 2, 2 * nx);
        break;
    case 3:
        zfp_field_set_stride_3d(field.get(), 2, 2 * nx, 2 * nx * ny);
        break;
    case 4:
        zfp_field_set_stride_4d(field.get(), 2, 2 * nx, 2 * nx * ny, 2 * nx * ny * nz);
        break;
    }
...
void compress_acquisition(ISMRMRD::ISMRMRD_Acquisition const &acq, std::vector<uint8_t> &buffer, unsigned int compression_precision, float compression_tolerance) {
    ...
    if (acq.head.active_channels > 1) {
        ...
        //       but that would require a Fourier transform of the signal. An alternatives would be the aligning the phase
...
void decompress_image(ISMRMRD::ISMRMRD_Image &image, std::vector<uint8_t> &buffer) {
    ...
    if (sx) {
        ...
        case 1:
            ...
            break;
        case 2:
            zfp_field_set_stride_2d(field.get(), sx, sx * nx);
            break;
        case 3:
            zfp_field_set_stride_3d(field.get(), sx, sx * nx, sx * nx * ny);
            break;
        case 4:
            zfp_field_set_stride_4d(field.get(), sx, sx * nx, sx * nx * ny, sx * nx * ny * nz);
            break;
        }
    }
