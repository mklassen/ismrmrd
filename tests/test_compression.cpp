...
typedef boost::mpl::list<uint16_t, int16_t, uint32_t, int32_t> test_promote_types;
...
void run_compression(uint nx, uint ny, uint nz, uint nw, zfp_type type) {
    ...
    zfp_field_set_pointer(field.get(), data1);
    ...
    BOOST_REQUIRE_NE(stream, nullptr);
    ...
    zfp_stream_rewind(zfp.get());
    zfp_field_set_pointer(field.get(), data2);
